    ...
    for (_, coords) in antenna_coords {
        for i in 0..coords.len() - 1 {
            for j in i + 1..coords.len() {
                ...
                let p1 = coords[i] - delta;
                let p2 = coords[j] + delta;

                if p1.x >= 0 && p1.x < rows && p1.y >= 0 && p1.y < cols as i32 { unique_nodes.insert(p1); }
                if p2.x >= 0 && p2.x < rows && p2.y >= 0 && p2.y < cols as i32 { unique_nodes.insert(p2); }
            }
        }
    }
