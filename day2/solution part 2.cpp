...
bool remove_one(const std::vector<int>& levels){
    for (size_t i{}; i < levels.size(); ++i){
        std::vector<int> temp = levels;
        temp.erase(temp.begin() + i);
        if (is_increasing_or_decreasing(temp) && atleast_one_atmost_three(temp)) return true;
    }
    return false;
}
...
int main() {
    ...
    for (const std::vector<int>& levels : reports){
        ...
        else if (remove_one(levels)) safe++;
    }
